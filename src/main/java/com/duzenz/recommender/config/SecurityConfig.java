...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().authorizeRequests().antMatchers("/login", "/login/form**", "/register", "/logout", "/rest/users/", "/rest/datauser/").permitAll() // #4
                .antMatchers("/admin", "/admin/**").hasRole("ADMIN") // #6
                .antMatchers("/adminprofile", "/adminprofile/**").hasRole("ADMIN") // #6
