...
public class ListeningResource {
	...
	@RequestMapping(value = "/foreign/", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public void f() {
		List<Artist> allArtist = artistDao.findAll();
		for (Artist artist : allArtist) {
			listeningDao.updateArtist(artist.getArtistId(), artist.getId());
		}
	}
	...
	@RequestMapping(value = "/listen/artist/{id}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public List<Listening> findListeningOfArtist(@PathVariable("id") int artistId) {
		return listeningDao.findListeningOfArtist(artistId);
	}
