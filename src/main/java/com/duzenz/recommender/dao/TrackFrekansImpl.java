...
public class TrackFrekansImpl implements TrackFrekansDao {
    ...
    public TrackFrekans findTrackFrekansWithTrackId(int trackId) {
        return em.createQuery("select a from TrackFrekans a where a.track.id = :trackId", TrackFrekans.class).setParameter("trackId", trackId).getSingleResult();
    }

    @Override
    @Transactional(readOnly = true)
    public List<TrackFrekans> findTopTracks(int trackCount) {
        return em.createQuery("select a from TrackFrekans a order by a.listenCount desc", TrackFrekans.class).setMaxResults(trackCount).getResultList();
    }

    @Override
    public List<TrackFrekans> filterTracksWithListenCount(int minCount, int maxCount) {
        return em.createQuery("select a from TrackFrekans a where a.listenCount >= :minCount and a.listenCount <= :maxCount order by a.listenCount asc", TrackFrekans.class).setParameter("minCount", minCount).setParameter("maxCount", maxCount).getResultList();
    }
